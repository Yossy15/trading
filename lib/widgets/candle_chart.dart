...
class _CandleChartState extends State<CandleChart> {
  ...
  Widget build(BuildContext context) {
    if (widget.candles.isEmpty) {
      return const Center(child: Text('No candle data', style: TextStyle(color: Colors.white54)));
    }

    final minLow = widget.candles.map((c) => c.low).reduce((a, b) => a < b ? a : b);
    final maxHigh = widget.candles.map((c) => c.high).reduce((a, b) => a > b ? a : b);
    ...
    String _formatAxisTime(String time) {
      ...
      if (widget.selectedTimeframe == 'D1' || widget.selectedTimeframe == 'W1') {
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              const Icon(Icons.candlestick_chart, color: Color(0xFFFFD700), size: 20),
          ...
          SizedBox(
            ...
            child: LayoutBuilder(
              builder: (context, constraints) {
                final availableWidth = constraints.maxWidth - 60; // 60 is for rightTitles
                  ...
                  bottomTitles: AxisTitles(
                    sideTitles: SideTitles(
                      ...
                      getTitlesWidget: (value, meta) {
                        ...
                        if (idx < 0 || idx >= widget.candles.length) return const SizedBox.shrink();
                ...
                gridData: FlGridData(
                  ...
                  getDrawingHorizontalLine: (value) {
                    return FlLine(color: Colors.white.withAlpha(15), strokeWidth: 1);
                  },
                ),
                borderData: FlBorderData(show: false),
                barGroups: _buildBarGroups(minY, maxY, bodyWidth, wickWidth),
                  ),
                );
              },
            ),
  ...
  Widget _buildPriceBadge() {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        color: isUp ? Colors.greenAccent.withAlpha(30) : Colors.redAccent.withAlpha(30),
        borderRadius: BorderRadius.circular(8),
        border: Border.all(
          color: isUp ? Colors.greenAccent.withAlpha(80) : Colors.redAccent.withAlpha(80),
        ),
      ),
      child: Text(
  ...
  List<BarChartGroupData> _buildBarGroups(double minY, double maxY, double bodyWidth, double wickWidth) {
